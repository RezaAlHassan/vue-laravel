      ...
      <h2 v-else>Edit Product</h2>
        <form @submit.prevent="submitForm">
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control" id="detail" v-model="product.detail" required></textarea>
          </div>
          <div class="mb-3">
  ...
  export default {
    ...
    async created() {
      if (!this.isNewProduct) {
        const response = await axios.get(`/api/products/${this.$route.params.id}`);//if this is not a new product
      ...
      async submitForm() {
        try {
          if(!this.product.detail) {
            console.log('The detail field is required');
          return;
          }
          if (this.isNewProduct) { //if it is a new product
